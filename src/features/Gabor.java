 ...
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOextensions.the
 * GNU General Public License for more details.
 * You would have received a copy of the GNU General Public License
 ...
 *     
 *  Modified by Ajay Bhat (to change Gabor Orientations etc)
 */
...
public class Gabor implements LireFeature {
	...
	private static int S = 4, T = 4; // filter mask size
	private static int M = 5, N = 6; // scale & orientation
	...
	static {
		...
		for (int i = 0; i < M; i++) {
			...
			sigma_x[i] = (A + 1) * Math.sqrt(2 * LOG2)
					/ (2 * Math.PI * Math.pow(A, i) * (A - 1) * U_L);
			sigma_y[i] = 1 / (2 * Math.PI * Math.tan(Math.PI / (2 * N)) * Math
					.sqrt(Math.pow(U_H, 2) / (2 * LOG2)
							- Math.pow(1 / (2 * Math.PI * sigma_x[i]), 2)));
		...
		for (int s = 0; s < S; s++) {
			for (int t = 0; t < T; t++) {
				for (int m = 0; m < M; m++) {
					for (int n = 0; n < N; n++) {
						selfSimilarGaborWavelet = gaborFeature
								.selfSimilarGaborWavelet(s, t, m, n);
	...
	public double getDistance(double[] targetFeatureVector,
			double[] queryFeatureVector) {
		double distance = 0;
		for (int m = 0; m < M; m++) {
			for (int n = 0; n < N; n++) {
				distance += Math
						.sqrt(Math.pow(queryFeatureVector[m * 2 * N + n * 2]
								- targetFeatureVector[m * 2 * N + n * 2], 2)
								+ Math.pow(queryFeatureVector[m * 2 * N + n * 2
										+ 1]
										- targetFeatureVector[m * 2 * N + n * 2
												+ 1], 2));
			}
		}
	...
	public double[] normalize(double[] featureVector) {
		...
		for (int m = 0; m < M; m++) {
			for (int n = 0; n < N; n++) {
				orientationVectorSum2 += Math.sqrt(Math.pow(featureVector[m * 2
						* N + n * 2], 2)
						+ Math.pow(featureVector[m * 2 * N + n * 2 + 1], 2));
			}
		...
		for (int m = dominantOrientation, k = 0; m < M; m++, k++) {
			for (int n = 0; n < N; n++) {
				normalizedFeatureVector[k * 2 * N + n * 2] = featureVector[m
						* 2 * N + n * 2];
				normalizedFeatureVector[k * 2 * N + n * 2 + 1] = featureVector[m
						* 2 * N + n * 2 + 1];
			}
		}
	...
	public double[] getFeature(BufferedImage image) {
		...
		int[][] grayLevel = new int[imageRaster.getWidth()][imageRaster
				.getHeight()];
		int[] tmp = new int[3];
			...
			for (int n = 0; n < N; n++) {
				...
				for (int x = S; x < image.getWidth(); x++) {
					for (int y = T; y < image.getHeight(); y++) {
						magnitudesForVariance[m][n] += Math.pow(
								Math.sqrt(Math.pow(this.gaborWavelet[x - S][y
										- T][m][n][0], 2)
										+ Math.pow(this.gaborWavelet[x - S][y
												- T][m][n][1], 2))
										- featureVector[m * 2 * N + n * 2], 2);
					}
				}

				featureVector[m * 2 * N + n * 2 + 1] = Math
						.sqrt(magnitudesForVariance[m][n]) / imageSize;
			}
		}
			...
			for (int n = 0; n < N; n++) {
				for (int x = S; x < image.length; x++) {
					for (int y = T; y < image[0].length; y++) {
						magnitudes[m][n] += Math
								.sqrt(Math
										.pow(this.gaborWavelet[x - S][y - T][m][n][0],
												2)
										+ Math.pow(this.gaborWavelet[x - S][y
												- T][m][n][1], 2));
	...
	// returns 2 doubles representing the real ([0]) and imaginary ([1]) part of
	// the mother wavelet
	...
	private double[] computeMotherWavelet(double x, double y, int m, int n) {
		...
		return new double[]{
				1
						/ (2 * Math.PI * sigma_x[m] * sigma_y[m])
						* Math.exp(-1
								/ 2
								* (Math.pow(x, 2) / Math.pow(sigma_x[m], 2) + Math
										.pow(y, 2) / Math.pow(sigma_y[m], 2)))
	...
	private double x_tilde(int x, int y, int m, int n) {
		return Math.pow(A, -m)
				* (x * Math.cos(theta[n]) + y * Math.sin(theta[n]));
	}

	private double y_tilde(int x, int y, int m, int n) {
		return Math.pow(A, -m)
				* (-x * Math.sin(theta[n] + y * Math.cos(theta[n])));
	}

	private double[] selfSimilarGaborWavelet(int x, int y, int m, int n) {
		double[] motherWavelet = computeMotherWavelet(x_tilde(x, y, m, n),
				y_tilde(x, y, m, n), m, n);
		return new double[]{Math.pow(A, -m) * motherWavelet[0],
	...
	public void setStringRepresentation(String s) { // added by mlux
		...
		if (!name.equals("gabor")) {
			throw new UnsupportedOperationException(
					"This is not a Gabor feature string.");
		}

		/*
		 * changes made by Ankit Jain here otherwise the histogram length would
		 * be assigned to histogram[i] jankit87@gmail.com
		 */
